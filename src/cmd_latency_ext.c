...
/* Redis 6: provide our own updateCommandLatencyHistogram (not available). */
static void updateCommandLatencyHistogram(struct hdr_histogram **h, int64_t duration) {
    ...
    if (*h == NULL) hdr_init(LATENCY_HISTOGRAM_MIN_VALUE, LATENCY_HISTOGRAM_MAX_VALUE,
...
void resetCommandExtendedTrackingRecursive(struct redisCommand *c) {
    ...
    /* Redis 6: no subcommands_dict; recursion not needed. */
}
