...
/* Update per-command extended stats (no aggregate update). */
static void updateCmdLatencyExtNoAggregate(struct redisCommand *cmd, long long duration_us) {
    if (!cmd) return;
...
void updateCommandExtendedLatencyStats(struct redisCommand *cmd, long long duration_us) {
    ...
    /* Propagate extended stats to parent chain so parent commands
     * (e.g., CONFIG) reflect subcommand latency (CONFIG|SET). */
    struct redisCommand *cur = cmd;
    while (cur) {
        updateCmdLatencyExtNoAggregate(cur, duration_us);
        cur = cur->parent;
    }
...
void resetCommandExtendedTracking(struct redisCommand *c) {
    if (!c) return;
    /* Fast path: skip when tracking was never used for this command. */
...
void resetCommandExtendedTrackingRecursive(struct redisCommand *c) {
    ...
    if (c->subcommands_dict) {
        dictIterator *di = dictGetSafeIterator(c->subcommands_dict);
        dictEntry *de;
...
static void initCommandTrackingFields(dict *commands) {
    ...
    while ((de = dictNext(di)) != NULL) {
        ...
        if (c->subcommands_dict)
            initCommandTrackingFields(c->subcommands_dict);
    }
    dictReleaseIterator(di);
}
