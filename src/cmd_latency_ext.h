 ...
 *
 * Redis 4/5 limitations:
 * - failed_calls: only tracked for wrong-type errors via call() path.
 *   Other error paths are not instrumented in Redis 4/5.
 * - rejected_calls: tracked for wrong-arity via processCommand().
 *   Other reject paths (OOM, read-only slave, LOADING, etc.) are
 *   not instrumented and will not appear in aggregate statistics.
 */

#ifndef __CMD_LATENCY_EXT_H
#define __CMD_LATENCY_EXT_H

/* Redis 4/5: unlikely() not globally defined in older versions. */
#ifndef unlikely
...
/* Cross-version portability macros (Redis 4/5/6/7) */
...
/* Reserved for Redis 6/7 portability -- used in those versions cmd_latency_ext.c */
 ...
 *
 * Memory: ~13.8 KiB per command when both histograms allocated
 *         (2.4KB window + ~11.4KB all-time histogram + ~11.4KB 1m histogram).
 *         Approximate total for ~200 commands: ~2.7 MiB when fully active.
 */
typedef struct {
    long long usec_min;
    long long usec_max;
